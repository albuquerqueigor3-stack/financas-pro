...
export default function App() {
  ...
  useEffect(()=>{
    const s = localStorage.getItem("fp_sess");
    if (s) setUser(s);
    setChecked(true);
  },[]);
      ...
      <div style={{padding:"14px",paddingBottom:"90px"}}>
          ...
          <div style={cardStyle}>
            ...
            {editInc?(
              <div style={{display:"flex",gap:"8px"}}>
                <input value={incInput} onChange={e=>setIncInput(e.target.value)} type="number" placeholder="0,00" style={{...inpStyle,flex:1,fontSize:"18px",border:"1px solid #6366f1"}} onKeyDown={e=>e.key==="Enter"&&saveInc()}/>
        ...
        {tab==="bills"&&<div>
          ...
          {showBF&&<div style={{...cardStyle,border:"1px solid #6366f1",marginBottom:"14px"}}>
            ...
            <select value={nb.category} onChange={e=>setNb({...nb,category:e.target.value})} style={{...inpStyle,marginBottom:"14px"}}>{CATS.map(c=><option key={c}>{c}</option>)}</select>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                ...
                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:"6px"}}>
                  ...
                  <div style={{display:"flex",gap:"4px"}}>
                    <button onClick={()=>togglePaid(b.id)} style={{...iBtn(paid?"#166534":"#1e3a5f",paid?"#4ade80":"#60a5fa"),padding:"4px 8px",fontSize:"11px",fontWeight:"700"}}>{paid?<><CheckCircle size={11}/>&nbsp;Pago</>:"Pagar"}</button>
        ...
        {tab==="exps"&&<div>
          ...
          {showEF&&<div style={{...cardStyle,border:"1px solid #6366f1",marginBottom:"14px"}}>
            ...
            <label style={lblStyle}>Categoria</label>
            <select value={ne.category} onChange={e=>setNe({...ne,category:e.target.value})} style={{...inpStyle,marginBottom:"14px"}}>{CATS.map(c=><option key={c}>{c}</option>)}</select>
            ...
            <div key={cat} style={{marginBottom:"10px"}}>
              ...
              {items.map(e=>(
                <div key={e.id} style={{...cardStyle,padding:"12px",marginBottom:"6px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div><div style={{fontWeight:"500",fontSize:"13px"}}>{e.name}</div><div style={{fontSize:"11px",color:"#64748b"}}>{fmtDate(e.date)}</div></div>
                  <div style={{display:"flex",gap:"8px",alignItems:"center"}}><span style={{fontWeight:"800",color:"#f43f5e"}}>{fmt(e.value)}</span><button onClick={()=>delExp(e.id)} style={iBtn("#450a0a","#f87171")}><Trash2 size={12}/></button></div>
                </div>
              ))}
            </div>
          ))}
        ...
        {tab==="cc"&&<div>
          ...
          <div style={cardStyle}>
            ...
            <input type="number" value={cc.total} onChange={e=>saveCC("total",e.target.value)} placeholder="0,00" style={{...inpStyle,fontSize:"22px",fontWeight:"800",border:"1px solid #a78bfa"}}/>
          </div>
          ...
          <div style={cardStyle}>
            ...
            {[["🚨","Pague SEMPRE o total. Rotativo chega a 400% ao ano."],["📊","Cartão é controle, não crédito extra."],["🎯","Parcele no máximo 3x."],["💰","Cashback só vale com fatura paga em dia."]].map((item,i)=>(
              <div key={i} style={{display:"flex",gap:"8px",marginBottom:"8px",paddingLeft:"10px",borderLeft:"2px solid #6366f1"}}><span>{item[0]}</span><span style={{fontSize:"12px",color:"#94a3b8",lineHeight:"1.5"}}>{item[1]}</span></div>
            ))}
          </div>
        </div>}
          ...
          {chartData.length===0?<div style={{textAlign:"center",color:"#475569",padding:"60px 0"}}>Sem dados em {MONTHS_PT[selMonth]}.</div>:<div>
            <div style={cardStyle}>
              ...
              <ResponsiveContainer width="100%" height={220}>
                <PieChart><Pie data={chartData} cx="50%" cy="50%" innerRadius={55} outerRadius={88} dataKey="value" label={({percent})=>Math.round(percent*100)+"%"} labelLine={false} fontSize={11}>
                  {chartData.map((_,i)=><Cell key={i} fill={COLORS[i%COLORS.length]}/>)}</Pie>
            ...
            <div style={cardStyle}>
              ...
              {chartData.slice(0,3).map((d,i)=>(
                <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"10px",paddingBottom:"10px",borderBottom:"1px solid #0f172a"}}>
                  <div><div style={{fontSize:"13px",fontWeight:"600"}}>{["🔴","🟡","🟢"][i]} {d.name}</div><div style={{fontSize:"11px",color:"#64748b"}}>{incNum>0?Math.round(d.value/incNum*100):0}% da renda</div></div>
            ...
            <div key={i} style={{...cardStyle,border:"1px solid "+(inv.star?"#16a34a":"#334155")}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"6px"}}>
                {[{l:"Risco",v:inv.risk},{l:"Mínimo",v:inv.min},{l:"Liquidez",v:inv.liq}].map(f=>(
                  <div key={f.l} style={{background:"#0f172a",borderRadius:"8px",padding:"7px",textAlign:"center"}}><div style={{fontSize:"10px",color:"#64748b",marginBottom:"2px"}}>{f.l}</div><div style={{fontSize:"11px",fontWeight:"700",color:"#e2e8f0",lineHeight:"1.3"}}>{f.v}</div></div>
                ))}
              </div>
            </div>
          ))}
      ...
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:"480px",background:"#1e293b",borderTop:"1px solid #334155",display:"flex",zIndex:20}}>
        {NAVS.map(n=>(
          <button key={n.id} onClick={()=>setTab(n.id)} style={{flex:1,background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:"2px",padding:"10px 0",color:tab===n.id?"#6366f1":"#475569",transition:"color .2s"}}>
            {n.icon}<span style={{fontSize:"9px",fontWeight:tab===n.id?"700":"400"}}>{n.label}</span>
          </button>
        ))}
      </div>
    </div>
